...
def linux_notification(icon, stime):
	...
	note = notify2.Notification('Done', message='Download Complete', icon = os.path.abspath(icon))
...
def notify():
	if platform.system() == "Window":
		...
		t1.start()
	if platform.system() == "Linux":
		t1 = threading.Thread(target=linux_notification, args=("./.kamaltorrent/kmt/images/icon.png", 15))
		t1.start()
